...
class _PythonRunner:
        ...
        with _working_directory(self._execution_cwd()):
            ...
            for event in self.document.events:
                if isinstance(event, CodeBlock):
                    ...
                    findings.append(self._verify_result(event))
        return findings
    ...
    def _verify_result(self, result: ResultAssertion) -> Finding:
        if not _is_python(result.language):
            return Finding(
                verifier_id=VERIFIER_ID,
                status=Status.SKIP,
                location=result.location,
                message=f"python verifier does not handle language {result.language!r}",
